...
def get_wind_data():
    ...
    wind_file = 'wind{}_file.csv'.format(year)
    ...
    try:
        ...
        print("before")
        wtk_file = '/nrel/wtk/conus/wtk_conus_{}.h5'.format(year)
        ...
        with WindX(wtk_file, hsds=True) as f:
            ...
            csv_data= data.to_csv('wind{}_file.csv'.format(year))  # Save the DataFrame to a CSV file
...
def get_solar_data():
    ...
    current_working_directory = os.getcwd()
    solar_file = 'solar{}_file.csv'.format(year)
    ...
    try:
        ...
        # Process the data as needed and prepare for JSON response
        # Note: You may need to convert the data from a DataFrame or numpy array into a JSON-serializable format
        # For example, if `data` is a pandas DataFrame:
